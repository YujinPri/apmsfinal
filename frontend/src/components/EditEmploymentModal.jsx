...
const EditEmploymentModal = ({ open, onClose, employmentID }) => {
  ...
  useEffect(() => {
    ...
    if (cachedData) {
      setEmploymentProfile({
        ...
        current_job: cachedData?.data?.date_end == null ? true : false,
        aligned_with_academic_program:
          cachedData?.data?.aligned_with_academic_program || false,
        date_end: dayjs(cachedData?.data.date_end) || null,
        classification: cachedData?.data?.classification || "",
        gross_monthly_income: cachedData?.data?.gross_monthly_income || "",
        employment_contract: cachedData?.data?.employment_contract || "",
        job_level_position: cachedData?.data?.job_level_position || "",
        type_of_employer: cachedData?.data?.type_of_employer || "",
        location_of_employment: cachedData?.data?.location_of_employment || "",
      });
    }
  }, [cachedData]);
  ...
  const handleSubmit = async (event) => {
    ...
    const data = {
      ...
      aligned_with_academic_program:
        employmentProfile?.aligned_with_academic_program,
            ...
            <Autocomplete
              ...
              value={
                classifications.find(
                  (option) => option.value === employmentProfile?.classification
                ) || null
              }
            />
          </Grid>
