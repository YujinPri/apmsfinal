...
class User(Base):
    ...
    # Remove the hashed_pass column, as password management is handled by Auth0.
    hashed_pass = Column(String)
    ...
    # Alumni-specific fields (all initially nullable)
    verified = Column(Boolean, default=None, nullable=True)
    student_number = Column(String, unique=True, nullable=True, default=None, index=True)
    ...
    barangay = Column(String, nullable=True, default=None)
    year_graduated = Column(Integer, nullable=True, default=None)
    telephone_number = Column(String, nullable=True, default=None)
    mobile_number = Column(String, nullable=True, default=None)
    civil_status = Column(String, nullable=True, default=None)
    ...
    civil_service_eligibility = Column(Boolean, nullable=True, default=None)
    present_employment_status = Column(String, nullable=True, default=None)
...
class SpecialSkillsCertification(Base):
    ...
    id = Column(Integer, primary_key=True)
    alumni_id = Column(Integer, ForeignKey('users.id', ondelete="CASCADE"))  # ForeignKey to link to alumni
...
class Employment(Base):
    ...
    id = Column(Integer, primary_key=True)
    alumni_id = Column(Integer, ForeignKey('users.id', ondelete="CASCADE"))
...
class Feedback(Base):
    ...
    id = Column(Integer, primary_key=True)
    alumni_id = Column(Integer, ForeignKey('users.id', ondelete="CASCADE"))
    users = relationship("User", back_populates="feedbacks")
